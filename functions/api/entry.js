...
// 统一入口，通过 ?action=parse|video|cover 参数路由，与 EdgeOne Pages 的 node-functions/api/entry.js 对齐。
// 前端统一只请求 /api/entry，避免平台差异导致路径不命中（从而 Pages fallback 到静态 index.html -> 前端报"服务端未返回JSON"）。
