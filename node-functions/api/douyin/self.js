// node-functions/api/douyin/self.js — EdgeOne Pages Node Function: /api/douyin/self
...
console.log('[douyin/self.js] 已强制使用 Node.js 原生 http 模块');
...
export async function onRequest(context) {
    ...
    console.log(`[douyin/self.js] onRequest start: ${request.method} /api/douyin/self${search}`);
    ...
    try {
        ...
        console.log('[douyin/self.js] buildParseResponse start');
        const t0 = Date.now();
        const result = await buildParseResponse(rawUrl);
        console.log('[douyin/self.js] buildParseResponse success, ms:', Date.now() - t0);
        const payload = result.payload || result;
        return jsonResponse(payload);
    } catch (e) {
        console.error('[douyin/self.js] ERROR:', e && e.message, e && e.stack);
